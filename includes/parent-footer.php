...
(function() {
    ...
    function openSidebar() { if (sidebar && overlay) { sidebar.classList.remove('-translate-x-full'); overlay.classList.add('open'); document.body.classList.add('overflow-hidden', 'lg:overflow-auto'); } }
    function closeSidebar() { if (sidebar && overlay) { sidebar.classList.add('-translate-x-full'); overlay.classList.remove('open'); document.body.classList.remove('overflow-hidden', 'lg:overflow-auto'); } }
...
(function() {
    ...
    if (userBtn && userDropdown) {
        userBtn.addEventListener('click', function(e) { e.stopPropagation(); userDropdown.classList.toggle('hidden'); document.getElementById('notif-dropdown').classList.add('hidden'); });
    ...
    function updateNotifCount() {
        fetch(BASE + '/modules/profile/notif-count.php').then(function(r) { return r.json(); }).then(function(d) { if (d.count > 0) { notifBadge.style.display = 'flex'; notifBadge.textContent = d.count > 99 ? '99+' : d.count; } else { notifBadge.style.display = 'none'; } });
    }
    function loadNotifications() {
        fetch(BASE + '/modules/profile/notif-list.php').then(function(r) { return r.json(); }).then(function(data) {
            if (!data || data.length === 0) { notifList.innerHTML = '<div class="p-6 text-center text-xs text-gray-400"><i class="fas fa-check-circle text-green-300 text-2xl mb-2 block"></i>No new notifications</div>'; return; }
            var html = '';
            ...
            notifList.querySelectorAll('.notif-item').forEach(function(item) {
                item.addEventListener('click', function() { var id = this.getAttribute('data-id'); if (id) { var f = new FormData(); f.append('id', id); fetch(BASE + '/modules/profile/notif-read.php', { method: 'POST', body: f }); this.classList.remove('bg-teal-50/50'); var dot = this.querySelector('span.w-2'); if (dot) dot.remove(); updateNotifCount(); } });
            });
        });
    }
