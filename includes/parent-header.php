...
<head>
    ...
    tailwind.config = {
        theme: {
            extend: {
                ...
                colors: {
                    coral: { 50: '#fef3f2', 100: '#fee4e2', 200: '#fecdca', 300: '#fda29b', 400: '#f97066', 500: '#f9645a', 600: '#e8453a', 700: '#c22f25' }
                }
            }
        }
    }
    ...
    <style>#app-loader{position:fixed;inset:0;z-index:9999;background:linear-gradient(135deg,#f0fdfa,#ffffff);display:flex;flex-direction:column;align-items:center;justify-content:center;transition:opacity .4s}#app-loader.hide{opacity:0;pointer-events:none}.ldr-book{width:70px;height:48px;position:relative;perspective:140px}.ldr-book .page{position:absolute;width:35px;height:48px;background:linear-gradient(135deg,#14b8a6,#0d9488);border-radius:3px;transform-origin:left center;animation:bookFlip 1.4s ease-in-out infinite;box-shadow:0 2px 8px rgba(13,148,136,.2)}.ldr-book .page:nth-child(2){animation-delay:.2s;background:linear-gradient(135deg,#2dd4bf,#0d9488)}.ldr-book .page:nth-child(3){animation-delay:.4s;background:linear-gradient(135deg,#5eead4,#14b8a6)}@keyframes bookFlip{0%,100%{transform:rotateY(0)}50%{transform:rotateY(-180deg)}}.ldr-text{margin-top:20px;font-size:13px;font-weight:600;letter-spacing:2px;color:#0d9488;text-transform:capitalize;animation:pulseText 1.6s ease-in-out infinite}@keyframes pulseText{0%,100%{opacity:.4}50%{opacity:1}}</style>
</head>
...
<div class="flex h-screen overflow-hidden">
    <aside id="sidebar" class="fixed lg:relative inset-y-0 left-0 z-40 w-64 bg-white border-r border-gray-200 flex flex-col -translate-x-full lg:translate-x-0 transition-transform duration-300">
        <div class="p-4 border-b border-gray-200 flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-teal-600 flex items-center gap-2">
                    ...
                    <a href="<?= BASE_URL ?>/modules/parent/dashboard.php">Ziada LMS</a>
                </h1>
                <p class="text-xs text-gray-400 mt-1">Parent Portal</p>
            </div>
        ...
        <nav class="flex-1 overflow-y-auto p-4 space-y-1">
            <a href="<?= BASE_URL ?>/modules/parent/dashboard.php" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 <?= basename($_SERVER['PHP_SELF']) == 'dashboard.php' ? 'bg-teal-50 text-teal-700 font-bold' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900' ?>">
                <i class="fas fa-th-large w-5 text-center"></i> Dashboard
            </a>
            <a href="<?= BASE_URL ?>/modules/parent/attendance.php" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 <?= basename($_SERVER['PHP_SELF']) == 'attendance.php' ? 'bg-teal-50 text-teal-700 font-bold' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900' ?>">
                <i class="fas fa-calendar-check w-5 text-center"></i> Attendance
            </a>
            <a href="<?= BASE_URL ?>/modules/parent/homework.php" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 <?= basename($_SERVER['PHP_SELF']) == 'homework.php' ? 'bg-teal-50 text-teal-700 font-bold' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900' ?>">
                <i class="fas fa-book-open w-5 text-center"></i> Homework
            </a>
            <a href="<?= BASE_URL ?>/modules/parent/tests.php" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 <?= basename($_SERVER['PHP_SELF']) == 'tests.php' ? 'bg-teal-50 text-teal-700 font-bold' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900' ?>">
                <i class="fas fa-flask w-5 text-center"></i> Test Results
            </a>
            <a href="<?= BASE_URL ?>/modules/parent/fees.php" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 <?= basename($_SERVER['PHP_SELF']) == 'fees.php' ? 'bg-teal-50 text-teal-700 font-bold' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900' ?>">
                <i class="fas fa-wallet w-5 text-center"></i> Fees
            </a>
            <a href="<?= BASE_URL ?>/modules/parent/notices.php" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 <?= basename($_SERVER['PHP_SELF']) == 'notices.php' ? 'bg-teal-50 text-teal-700 font-bold' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900' ?>">
                <i class="fas fa-bullhorn w-5 text-center"></i> Notices
            </a>
            <a href="<?= BASE_URL ?>/modules/parent/leave.php" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 <?= basename($_SERVER['PHP_SELF']) == 'leave.php' ? 'bg-teal-50 text-teal-700 font-bold' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900' ?>">
                <i class="fas fa-calendar-alt w-5 text-center"></i> Apply Leave
            </a>
            ...
            <div class="flex items-center gap-4">
                <div class="relative" id="notif-container">
                    ...
                    <div id="notif-dropdown" class="hidden absolute right-0 top-full mt-2 w-80 bg-white rounded-xl border border-gray-200 shadow-xl z-50 overflow-hidden">
                        ...
                        <a href="<?= BASE_URL ?>/modules/parent/notices.php" class="block px-4 py-2.5 text-center text-xs font-medium text-teal-600 hover:bg-teal-50 border-t border-gray-100 transition">
                ...
                <div class="relative" id="user-menu-container">
                    ...
                    <div id="user-dropdown" class="hidden absolute right-0 top-full mt-2 w-48 bg-white rounded-xl border border-gray-200 shadow-xl z-50 overflow-hidden">
                        <a href="<?= BASE_URL ?>/modules/parent/profile.php" class="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition">
