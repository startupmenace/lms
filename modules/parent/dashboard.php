...
$children = db_get_all(
    "SELECT s.*, c.name as class_name
     ...
     WHERE sp.parent_user_id = ? AND s.is_active = 1
     ORDER BY s.parent_name", [$user_id]);
...
if (empty($children)) {
    $children = db_get_all(
        "SELECT s.*, c.name as class_name
         ...
         LEFT JOIN classes c ON s.class_id = c.id
         WHERE s.id IN (
             SELECT sp.student_id FROM student_parents sp WHERE sp.parent_user_id = ?
         ) AND s.is_active = 1 ORDER BY s.parent_name",
        [$user_id]);
}
...
foreach ($children as $c) {
    if ($c['id'] == $child_id) { $selected = $c; break; }
}
...
$recent_homework = db_get_all(
    "SELECT h.*, s.name as subject_name
     FROM homework h
     LEFT JOIN subjects s ON h.subject_id = s.id
     WHERE h.class_id = ? ORDER BY h.created_at DESC LIMIT 5",
    [$selected['class_id'] ?? 0]);

$recent_tests = db_get_all(
    "SELECT ts.*, t.title as test_title, t.total_marks
     FROM test_submissions ts
     LEFT JOIN tests t ON ts.test_id = t.id
     WHERE ts.student_id = ? ORDER BY ts.submitted_at DESC LIMIT 5",
    [$student_id]);

$notices = db_get_all(
    "SELECT * FROM notices WHERE (target_audience IN ('all','student') OR class_id = ?) AND is_active = 1 ORDER BY created_at DESC LIMIT 4",
    [$selected['class_id'] ?? 0]);

$fees = db_get_all(
    "SELECT t.*, fs.name as structure_name FROM transactions t
     LEFT JOIN fee_structures fs ON t.fee_structure_id = fs.id
     WHERE t.student_id = ? ORDER BY t.created_at DESC LIMIT 4",
    [$student_id]);
...
<div class="mb-6 flex items-center gap-3 flex-wrap">
    ...
    <a href="?child_id=<?= $c['id'] ?>"
       class="px-4 py-2 rounded-lg text-sm font-medium transition <?= $c['id'] == $child_id ? 'bg-teal-600 text-white shadow' : 'bg-white border border-gray-200 text-gray-700 hover:bg-gray-50' ?>">
        <?= sanitize($c['parent_name']) ?>
        <span class="text-xs opacity-75">(<?= sanitize($c['class_name'] ?? 'N/A') ?>)</span>
    </a>
    <?php endforeach; ?>
</div>
<?php endif; ?>
...
<div class="text-center py-16 bg-white rounded-xl border border-gray-200">
    <div class="w-16 h-16 mx-auto bg-gray-100 rounded-full flex items-center justify-center mb-4">
        <i class="fas fa-child text-2xl text-gray-400"></i>
    </div>
    <h3 class="text-lg font-semibold text-gray-900 mb-1">No children linked</h3>
    <p class="text-sm text-gray-500">Contact the school to link your account to your children.</p>
</div>
<?php include __DIR__ . '/../../includes/parent-footer.php'; return; ?>
<?php endif; ?>
    ...
    <div class="bg-white rounded-xl p-6 border border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900 mt-1"><?= count($recent_tests) ?></p>
            </div>
    ...
    <div class="bg-white rounded-xl p-6 border border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900 mt-1"><?= count($recent_homework) ?></p>
            </div>
            <div class="w-12 h-12 bg-coral-100 rounded-lg flex items-center justify-center">
                <i class="fas fa-book-open text-xl text-coral-600"></i>
            </div>
        </div>
        <div class="mt-3 text-xs text-gray-500">Pending assignments</div>
    </div>
    <div class="bg-white rounded-xl p-6 border border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm text-gray-500">Class</p>
                <p class="text-2xl font-bold text-gray-900 mt-1"><?= sanitize($selected['class_name'] ?? 'N/A') ?></p>
            </div>
            <div class="w-12 h-12 bg-teal-100 rounded-lg flex items-center justify-center">
                <i class="fas fa-school text-xl text-teal-600"></i>
            </div>
        </div>
        <div class="mt-3 text-xs text-gray-500"><?= sanitize($selected['parent_name']) ?></div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="font-semibold text-gray-900"><i class="fas fa-book-open text-coral-600 mr-2"></i>Homework</h3>
            <a href="homework.php<?= $child_id ? '?child_id='.$child_id : '' ?>" class="text-sm text-teal-600 hover:underline">View All</a>
        </div>
        <?php if (empty($recent_homework)): ?>
        <p class="text-sm text-gray-400 text-center py-6">No homework assigned</p>
        <?php else: ?>
        <div class="space-y-3">
            <?php foreach ($recent_homework as $h): ?>
            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                    <p class="text-sm font-medium text-gray-900"><?= sanitize($h['title']) ?></p>
                    <p class="text-xs text-gray-500"><?= sanitize($h['subject_name'] ?? 'General') ?> | Due: <?= $h['due_date'] ? format_date($h['due_date']) : 'No due date' ?></p>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="tests.php<?= $child_id ? '?child_id='.$child_id : '' ?>" class="text-sm text-teal-600 hover:underline">View All</a>
        </div>
        <?php if (empty($recent_tests)): ?>
        <p class="text-sm text-gray-400 text-center py-6">No results yet</p>
        <?php else: ?>
        <div class="space-y-3">
            <?php foreach ($recent_tests as $sub): ?>
            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="fees.php<?= $child_id ? '?child_id='.$child_id : '' ?>" class="text-sm text-teal-600 hover:underline">View All</a>
        </div>
        <?php
